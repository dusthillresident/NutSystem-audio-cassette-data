...
# If you want to use this script, you'll probably need to work out what specific options you need to give to arecord for your particular setup.
 ...
 arecord -I -r 44100 -t raw -f U8 -c 2 /tmp/readin /dev/null &
# arecord -D hw:1 -c 1 -r 44100 -t raw -f S16_LE > /tmp/readin &

# This one is suitable for use with cheap & common USB-audio adapters. The ones I have only support recording 16bit little endian mono. The dec_win program normally can only use 8bit audio. In order to use this arecord command, you will need to edit the signal_filter file and set the variable 'converts16%' to 'TRUE', and you must make sure that the converts16% option is set to FALSE in dec_win
...
brandy -quit signal_filter & 
brandy $1 dec_win 
killall -q arecord
sleep 0.1s && killall -q brandy &
rm /tmp/readdata
rm /tmp/readin
